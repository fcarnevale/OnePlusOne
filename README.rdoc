...
2. Startup postgresql on your system, and create a local db

3. Run <tt>rake db:schema:load</tt> to load the db schema

4. Run <tt>rails console</tt>, and create yourself a user (requires a name, email, password, and password_confirmation)

5. Run <tt>rails s</tt> to start the server, and then login and begin adding teams/people

OR

Just go to http://oneplusone.herokuapp.com, and login with the credentials I emailed to you
...
I ended up using a has_many through relationship between people and a partnership model. Each person has many partners through the partnerships model. The partnerships table consists mainly of a partner id and a person id.
...
If you navigate to /partnerships on the Heroku site (while you're signed in), you'll see the view that I used to confirm that the weekly pairings were adhering to the rules set out in the gist.
